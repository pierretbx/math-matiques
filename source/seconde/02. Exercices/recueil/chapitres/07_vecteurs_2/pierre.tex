...
\item Dans le repère $(O,\overrightarrow{i},\overrightarrow{j})$, lire les coordonnées des vecteurs $\overrightarrow{a}$ à $\overrightarrow{h}$.
\item Choisir trois de ces vecteurs et les écrire sous la forme $x\overrightarrow{i}+y\overrightarrow{j}$.
...
\[
2\overrightarrow{a}\ ;\quad -3\overrightarrow{f}\ ;\quad \overrightarrow{c}+2\overrightarrow{h}\ ;\quad -\overrightarrow{d}+4\overrightarrow{b}.
\]
...
\exo
Dans un repère, on donne les points $A(5;8)$, $B(3;2)$ et $C(2;9)$.

\begin{enumerate}
\item Calculer les coordonnées des vecteurs $\overrightarrow{AB}, \overrightarrow{BC}$ et $\overrightarrow{AC}$.
\item Calculer les coordonnées du vecteur $\overrightarrow{AB}+\overrightarrow{BC}$.
\item Expliquer le résultat obtenu.
\end{enumerate}

\exo
Dans un repère, on donne un vecteur $\overrightarrow{u} \left( \begin{array}{c}
1\\
3
\end{array} \right)$ et un point $A(2;0)$. Déterminer les coordonnées du point $M$ tel que $\overrightarrow{AM}=\overrightarrow{u}$.

\exo
Dans chaque cas, déterminer à l'aide du déterminant si les vecteurs $\overrightarrow{u}$ et $\overrightarrow{v}$ sont colinéaires.
...
6
\end{array} \right)$
Démontrer qu'ils sont colinéaires, puis calculer leurs normes.

\exo
Dans le repère du premier exercice, calculer la norme des vecteurs $\overrightarrow{a}$ à $\overrightarrow{h}$.

\exo
Dans un repère orthonormé, on donne les points $A(-3;2)$, $B(3;1)$, $C(5;2)$ et $D(0;-4)$.
Calculer les longueurs $AB$, $CD$ et $BC$.


\exo
Dans un repère orthonormé, on donne les points $A(-3;4)$, $B(0;2)$ et $C(4;4)$.
Calculer les coordonnées des milieux $K$ de $[AB]$, $L$ de $[BC]$ et $M$ de $[AC]$.
...
\item Calculer les longueurs de ses diagonales.
\end{enumerate}

\exo
Dans un repère, on donne les points $A(1;-1)$, $B(0,5;1,5)$ et $C(x;4)$.
On cherche la valeur de $x$ pour laquelle les points $A$, $B$ et $C$ sont alignés.
\begin{enumerate}
\item Quelle condition doivent vérifier les vecteurs $\overrightarrow{AB}$ et $\overrightarrow{BC}$ pour que les points $A$, $B$ et $C$ soient alignés ?
\item Déterminer $x$ pour que les points $A$, $B$ et $C$ soient alignés.
\end{enumerate}
